...
describe Twilio::SMS do
  ...
  def new_sms_should_have_been_made
    request(:post, sms_resource_uri).with(:body => minimum_sms_params).should have_been_made
  end
  ...
  describe "#[]" do
    let(:sms) { Twilio::SMS.new(:if_machine => :continue) }
    it 'is a convenience for accessing attributes' do
      sms['IfMachine'].should == 'Continue'
    end
    ...
    it 'is agnostic as to whether the attributes are accessed using the symbol style, e.g. :if_machine or the Twilio string style, e.g. "IfMachine"' do
      sms[:if_machine].should == 'Continue'
    end
  end
  ...
  describe 'behaviour on API error' do
    it 'raises an exception' do
      ...
      stub_request(:post, sms_resource_uri).with(:body => minimum_sms_params).to_return :body => canned_response('api_error'), :status => 404
